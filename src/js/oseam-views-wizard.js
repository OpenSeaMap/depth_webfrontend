...
this.setModel();  	  
 ...
 if (this.currentView.validate()) {	
  ...
  var currentStep = this.options.steps[this.currentStep];
	  ...
	  {
	  case 1:
		    	this.model.set({name: $('#configname').val()});
				this.model.set({description: $('#description').val()});
			    break;
	  case 2:
	  	  if ($('#echoSounderRightOf').prop("checked") == true){
	  	  this.model.set({echoSounderRightOf: 'checked'});
		   	  } else { 
	  this.model.set({echoSounderRightOf: ''});
	  }  
	  if ($('#echoSounderInFront').prop("checked") == true){
	 	  this.model.set({echoSounderInFront: 'checked'});
	  	  } else { 
	  this.model.set({echoSounderInFront: ''});
	  }
	  this.model.set({distanceY: $('#distanceY').val()});
	  this.model.set({distanceX: $('#distanceX').val()});
		  
	    break;
	  case 3:
	  	this.model.set({slidingspeed: $('#slidingspeed').val()});

		 		if ($('#sailingYacht').prop("checked") == true){
	 	  this.model.set({sailingYacht: 'checked'});
	  	  } else { 
	  this.model.set({sailingYacht: ''});
	  } 
	  		if ($('#motorYacht').prop("checked") == true){
	 	  this.model.set({motorYacht: 'checked'});
	  	  } else { 
	  this.model.set({motorYacht: ''});
	  } 
	  		if ($('#motorYachtDisplacer').prop("checked") == true){
	 	  this.model.set({motorYachtDisplacer: 'checked'});
	  	  } else { 
	  this.model.set({motorYachtDisplacer: ''});
	  } 
		    break;
				case 4:
		  		     
	  this.model.set({idDepthMeasured: $('#idDepthMeasured').val()});
	  this.model.set({idDepthDisplayed: $('#idDepthDisplayed').val()});
	    break;		
	  }
